...
```{r jerc_ndni, fig.path="figures/jerc_ndni"}
...
avgN = c(1.77887753,  1.3581499 ,  1.7286327 ,  1.6058137 ,  1.61899053,1.4614277 ,  1.6298325 ,  1.55414365,  1.52968035,  1.28296597,1.6493719 ,  2.1887554 ,  1.71025765,  1.33459525,  1.33841715,1.5472149 ,  1.4920822))
...
avgN = c( 1.46363636,  1.375     ,  1.6375    ,  1.875     ,  1.87142857,1.32      ,  1.2125    ,  1.33333333,  1.4       ,  1.9       , 1.38      ,  1.6       ,  1.51666667,  1.24      ,  2.2))
...
osbs_ndni <- data.frame(ndni = c(0.05222636,  0.0351661 ,  0.05479124,  0.02884115,  0.03701876,
        ...
        0.03247082,  0.03232074,  0.02887205,  0.04684527), avgN = c(1.8511326 ,  0.7318521 ,  1.31530557,  1.05164845,  1.68218235,1.0794374 ,  1.1535965 ,  0.6706812 ,  2.107867  ,  1.532048  ,1.10755733,  1.32534795,  1.92002763,  1.5034832 ,  0.6828276 ,0.8406297 ,  0.96103   ,  0.8184837 ,  0.9669018 ))

ggplot(jerc_ndni, aes(ndni, avgN)) + geom_point(size=3.5) + theme_bw() + stat_smooth(method = "lm", size =2, alpha = 0.3) + annotate("text", x = c(0.045, 0.045), y = c(2,1.9), label = c("R2 = 0.0", "p = 0.85"), size = 12) + xlab("Average NDNI per plot") + ylab("Average %N per plot") + ggtitle("NDNI vs. Foliar N: JERC")
```

```{r sjer_ndni, fig.path="figures/sjer_ndni"}
ggplot(sjer_ndni, aes(ndni, avgN)) + geom_point(size=3.5) + theme_bw() + stat_smooth(method = "lm", size =2, alpha = 0.3) + annotate("text", x = c(0.03, 0.03), y = c(2,1.9), label = c("R2 = 0.12","p = 0.21"), size = 12) + xlab("Average NDNI per plot") + ylab("Average %N per plot") + ggtitle("NDNI vs. Foliar N: SJER")

```

```{r osbs_ndni, fig.path="figures/osbs_ndni"}
ggplot(osbs_ndni, aes(ndni, avgN)) + geom_point(size=3.5) + theme_bw() + stat_smooth(method = "lm", size =2, alpha = 0.3) + annotate("text", x = c(0.045,0.045), y = c(1.8,1.70), label = c("R2 = 0.21", "p = 0.05"), size = 12) + xlab("Average NDNI per plot") + ylab("Average %N per plot") + ggtitle("NDNI vs. Foliar N: OSBS")
